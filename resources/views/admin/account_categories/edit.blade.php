...
@section('title', 'Chỉnh sửa danh mục tài khoản')
...
<div class="py-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mt-6 bg-white shadow overflow-hidden sm:rounded-lg">
            <form action="{{ route('admin.account_categories.update', $accountCategory->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="px-4 py-5 sm:p-6">
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <div class="mt-2 mb-3">
                                <img src="{{ asset('storage/' . $accountCategory->image) }}" alt="{{ $accountCategory->name }}" class="h-32 w-auto object-cover rounded-md">
                                <p class="mt-1 text-xs text-gray-500">Hình ảnh hiện tại</p>
                            </div>
                            @endif
                            ...
                            @error('image')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                            <p class="mt-1 text-xs text-gray-500">Để trống nếu không muốn thay đổi hình ảnh.</p>
                        </div>
                        
                        <div class="flex flex-col space-y-4">
                            <div class="flex items-center">
                                <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $accountCategory->is_active) ? 'checked' : '' }}
                            ...
                            <div class="flex items-center">
                                <input type="checkbox" id="is_featured" name="is_featured" value="1" {{ old('is_featured', $accountCategory->is_featured) ? 'checked' : '' }}
                                       class="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
